...
func newField(owner IBusinessObjectModel, name string, multiple bool, propType propertyType) field {
	return field{
	...
	if len(interfaceImplementations[srcClassName]) == 0 {
		...
		for _, class := range core.GetSortedValues(classRegistry.items) {
			if !class.isInterface() {
				if boType := utils.TypeOf(class.NewObject(), false); boType.Implements(targetFldTyp) {
