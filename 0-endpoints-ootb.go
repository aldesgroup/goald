...
func HandleGetAll[ResourceType IBusinessObject](webCtx WebContext) ([]ResourceType, hstatus.Code, string) {
	list, errList := LoadBOs[ResourceType](webCtx.GetBloContext(), webCtx.GetResourceClass(), webCtx.GetResourceLoadingType())
	if errList != nil {
		msg := ErrorC(errList, "Could not get a list of '%s' instances", webCtx.GetResourceClass().base().name).Error()
