...
// for now, we consider a unique batch size for inserts,
const batchSize = 1000

// ------------------------------------------------------------------------------------------------
// Batch insert - shared logic reused by every generated DAO's ExecCreateQuery
// ------------------------------------------------------------------------------------------------

// ExecCreateContext gathers everything that's specific to one business object model's insert query
type ExecCreateContext struct {
...
func (baseDAO *BusinessObjectDAO) ExecBatchCreate(ctx *ExecCreateContext) (map[int]int64, error) {
	...
	for start := 0; start < len(ctx.BObjs); start += batchSize {
		...
		// ExecBatchCreate - otherwise every batch's rows would stay open until the whole function returns
...
// ------------------------------------------------------------------------------------------------
// Batch link insert - shared logic reused by every generated DAO's ExecCreateLinksQueries
// ------------------------------------------------------------------------------------------------

// ExecCreateLinkContext gathers everything that's specific to one relationship's link table
type ExecCreateLinkContext struct {
...
// ExecBatchCreateLink performs a batched, multi-row insert of all the link rows described by the given context
func (baseDAO *BusinessObjectDAO) ExecBatchCreateLink(ctx *ExecCreateLinkContext) error {
